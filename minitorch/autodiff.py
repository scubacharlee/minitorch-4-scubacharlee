...
class FunctionBase:
    ...
    def chain_rule(cls, ctx, inputs, d_output):
        ...
        return [VariableWithDeriv(inp, d_input)
...
def backpropagate(final_variable_with_deriv):
    ...
    """
    q = [final_variable_with_deriv]
    while q:
        curr = q[0]
        var = curr.variable
        q = q[1:]
        if is_leaf(var):  # Is leaf node
            curr.variable._add_deriv(curr.deriv)
        else:
            for prev in var.history.backprop_step(curr.deriv):
                q.append(prev)
